...
public class GetMarketDataCoinGeckoCommand : ICommand<CoinGeckoCoinWithMarketDataRequestDto, IActionResult>
{
    ...
    public GetMarketDataCoinGeckoCommand(
        ...
        IConfiguration configuration)
    {
        ...
        _routingKey = configuration["RABBITMQ_COINGECKO_MARKETDATA_PROCESSOR_QUEUE_ROUTING_KEY"]!;
    }

    public async Task<IActionResult> ExecuteAsync(CoinGeckoCoinWithMarketDataRequestDto request)
    {
        var response = await _coinGeckoApiService.GetMarketDataByIdAsync(request);
        await _rabbitMQPublisherService.PublishAsync(response, _routingKey);

        return new object().ToHttpResponse("Market data from CoinGecko Api fetched successfully", 200);
    }
}
