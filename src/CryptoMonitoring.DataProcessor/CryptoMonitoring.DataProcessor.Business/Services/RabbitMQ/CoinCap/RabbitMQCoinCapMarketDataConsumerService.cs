...
public class RabbitMQCoinCapMarketDataConsumerService : BackgroundService
{
    private readonly IChannel _channel;
    private readonly string _queueName;
    private readonly IServiceScopeFactory _scopeFactory;

    public RabbitMQCoinCapMarketDataConsumerService(
        IRabbitMQConnectionFactory factory,
        IConfiguration configuration,
        IServiceScopeFactory scopeFactory)
    {
        _channel = factory.GetChannel();
        _queueName = configuration["RABBITMQ_COINCAP_MARKETDATA_PROCESSOR_QUEUE_NAME"]!;
        _scopeFactory = scopeFactory;
    }

    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        Log.Information("Starting CoinCap market data consumer background service");
        ...
        consumer.ReceivedAsync += async (model, ea) =>
        {
            try
            {
                ...
                Log.Information("CoinCap market data received successfully");

                await coinCapProcessingService.ProcessMarketDataAsync(json!);

                await _channel.BasicAckAsync(ea.DeliveryTag, false);
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Error processing CoinCap market data");
